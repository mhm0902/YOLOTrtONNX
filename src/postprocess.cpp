...
template<typename T> void print2file(T *p, const char *filename, int w, int h, int datalen=0)
{
    // std::ofstream oufile(filename, std::ios::app);
    ...
    for (int r = 0; r < h; r++)
    {
        for (int c = 0; c < w; c++)
        {
            ...
            else
            {
                if(c==w-1)
                {
                    oufile << std::fixed << std::setprecision(5) << p[r*w + c] << "\n";
                }else
                {
                    oufile << std::fixed << std::setprecision(5) << p[r*w + c] << ",";
                }
...
void nms(std::vector<Detection>& res, float* output, float conf_thresh, float nms_thresh, nvinfer1::Dims out_dims) {
    int det_size = sizeof(Detection) / sizeof(float);
    std::map<float, std::vector<Detection>> m;
    int signalResultNum = out_dims.d[2]; //84
    int strideNum = out_dims.d[1]; //8400

    // print2file(output, "./output.txt", 84, 8400);
    for (int i = 0; i < strideNum; ++i)
    {
        float* classesScores = output + 4;
        cv::Mat scores(1, kNumClass, CV_32FC1, classesScores);
        cv::Point class_id;
        ...
        if (maxClassScore > conf_thresh)
        {
            ...
            float w2 = output[2]/2;
            float h2 = output[3]/2;
            det.bbox[0] = output[0] - w2;
            det.bbox[1] = output[1] - h2;
            det.bbox[2] = output[0] + w2;
            det.bbox[3] = output[1] + h2;
            det.conf    = maxClassScore;
            det.class_id= class_id.x;
            if (m.count(det.class_id) == 0)
                m.emplace(det.class_id, std::vector<Detection>());
            m[det.class_id].push_back(det);
        }
        output += signalResultNum;
    }
...
void batch_nms(std::vector<std::vector<Detection>>& res_batch, float* output, int batch_size, int output_size, nvinfer1::Dims out_dims,
               float conf_thresh, float nms_thresh) {
    res_batch.resize(batch_size);
    for (int i = 0; i < batch_size; i++) {
        nms(res_batch[i], &output[i * output_size], conf_thresh, nms_thresh, out_dims);
    }
}
