...
const int kOutcount = (kNumClass+4) * 8400;
int signalResultNum = kNumClass + 4; //84
static __global__ void decode_kernel(float* predict, int num_bboxes, float confidence_threshold, float* parray,
    ...
    float* pitem = predict + position * 84;


    float* confidence = pitem + 4;
    float class_confidence = confidence[0];
    // for循环判断是哪个类别
    int label = 0;
    for (int i = 1; i < kNumClass; i++)
    {
        if (class_confidence < confidence[i])
        {   
    ...
    float w2 = pitem[2]/2;
    float h2 = pitem[3]/2;
    float left = pitem[0] - w2;
    float top = pitem[1] - h2;
    float right = pitem[0] + w2;
    float bottom = pitem[1] + h2;
    // float label = pitem[5];
