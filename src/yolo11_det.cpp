...
// const int kOutputSize = (kNumClass+4) * 8400;
...
void prepare_buffer(ICudaEngine* engine, float** input_buffer_device, float** output_buffer_device,
    ...
    int kOutputSize = out_dims.d[1]*out_dims.d[2];
...
void infer(IExecutionContext& context, cudaStream_t& stream, void** buffers, float* output, int batchsize,
           float* decode_ptr_host, float* decode_ptr_device, int model_bboxes, std::string cuda_post_process, int kOutputSize) {
...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < file_names.size(); i += kBatchSize) {
        ...
        // Run inference
        // memset(output_buffer_host, 0, kBatchSize * kOutputSize*sizeof(float));
        infer(*context, stream, (void**)device_buffers, output_buffer_host, kBatchSize, decode_ptr_host,
              decode_ptr_device, model_bboxes, cuda_post_process, kOutputSize);
        ...
        if (cuda_post_process == "c") {
            // NMS
            batch_nms(res_batch, output_buffer_host, img_batch.size(), kOutputSize, out_dims, kConfThresh, kNmsThresh);
        } else if (cuda_post_process == "g") {
